...
g.Application = Class.extend({
    ...
    draw: function(arr) {
        var bionum = arr.input.length + arr.gene.length + arr.output.length;
        ...
        //
        this.view.addFigure(this.label, x - this.label.getWidth() / 2, y - this.label.getHeight() / 2);
        var index = 0;
        for (var i = 0; i < arr.input.length; ++i, ++index) {
            var bio = new g.Shapes.Biobrick(arr.input[i]);
            this.view.addFigure(bio, x + Math.sin(2.0 * parseFloat(index) * 3.1415926 / parseFloat(bionum)) * radius, y - Math.cos(2.0 * parseFloat(index) * 3.1415926 / parseFloat(bionum)) * radius);
            console.log(Math.sin(parseFloat(index) * 3.1415926 / parseFloat(bionum)));
        }
        for (var i = 0; i < arr.gene.length; ++i, ++index) {
            var bio = new g.Shapes.Biobrick(arr.gene[i]);
            this.view.addFigure(bio, x + Math.sin(2.0 * parseFloat(index) * 3.1415926 / parseFloat(bionum)) * radius, y - Math.cos(2.0 * parseFloat(index) * 3.1415926 / parseFloat(bionum)) * radius);
        }
        for (var i = 0; i < arr.output.length; ++i, ++index) {
            var bio = new g.Shapes.Biobrick(arr.output[i]);
            this.view.addFigure(bio, x + Math.sin(2.0 * parseFloat(index) * 3.1415926 / parseFloat(bionum)) * radius, y - Math.cos(2.0 * parseFloat(index) * 3.1415926 / parseFloat(bionum)) * radius);
        }
    }
});
